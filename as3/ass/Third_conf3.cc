...
NS_LOG_COMPONENT_DEFINE ("Third_conf1ScriptExample");
...
main (int argc, char *argv[])
{
  ...
  address2.SetBase ("10.1.1.128", "255.255.255.128");
  ...
  TypeId tid0 = TypeId::LookupByName ("ns3::TcpNewRenoCSE");
  Config::Set ("/NodeList/*/$ns3::TcpL4Protocol/SocketType", TypeIdValue (tid0));
  Ptr<Socket> ns3TcpSocket1 = Socket::CreateSocket (nodes.Get (0), TcpSocketFactory::GetTypeId ());
  Ptr<MyApp> app1 = CreateObject<MyApp> ();
  app1->Setup (ns3TcpSocket1, sinkAddress1, 3000, 1000, DataRate ("1Mbps"));
  ...
  app2->Setup (ns3TcpSocket2, sinkAddress2, 3000, 1000, DataRate ("1Mbps"));
  ...
  app3->Setup (ns3TcpSocket3, sinkAddress3, 3000, 1000, DataRate ("1Mbps"));
  nodes.Get (1)->AddApplication (app3);
  app3->SetStartTime (Seconds (15.));
  app3->SetStopTime (Seconds (30.));

  AsciiTraceHelper asciiTraceHelper1;
  Ptr<OutputStreamWrapper> stream1 = asciiTraceHelper1.CreateFileStream ("Third_conf3_1.cwnd");
  ...
  Ptr<OutputStreamWrapper> stream2 = asciiTraceHelper2.CreateFileStream ("Third_conf3_2.cwnd");
  ...
  Ptr<OutputStreamWrapper> stream3 = asciiTraceHelper3.CreateFileStream ("Third_conf3_3.cwnd");
  ...
  plotHelper.ConfigurePlot ("Third_conf3_1-packet-byte-count",
  ...
  // for the output files generated.  The second, third, and fourth
  // arguments are, respectively, the plot title, x-axis, and y-axis labels
  plotHelper1.ConfigurePlot ("Third_conf3_2-packet-byte-count",
                            "Packet Byte Count vs. Time",
                            "Time (Seconds)",
                            "Packet Byte Count");
  ...
  // specifies the name of the data series label on the plot.  The last
  // argument formats the plot by specifying where the key should be placed.
  plotHelper1.PlotProbe (probeType,
                        tracePath,
                        "OutputBytes",
                        "Packet Byte Count",
                        GnuplotAggregator::KEY_BELOW);
  ...
  // for the output files generated.  The second, third, and fourth
  // arguments are, respectively, the plot title, x-axis, and y-axis labels
  plotHelper2.ConfigurePlot ("Third_conf3_3-packet-byte-count",
                            "Packet Byte Count vs. Time",
                            "Time (Seconds)",
                            "Packet Byte Count");
  ...
  // specifies the name of the data series label on the plot.  The last
  // argument formats the plot by specifying where the key should be placed.
  plotHelper2.PlotProbe (probeType,
                        tracePath,
                        "OutputBytes",
                        "Packet Byte Count",
                        GnuplotAggregator::KEY_BELOW);
  ...
  Ptr<PcapFileWrapper> file1 = pcapHelper1.CreateFile ("Third_conf1_1.pcap", std::ios::out, PcapHelper::DLT_PPP);
  devices1.Get (1)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop, file1));

  PcapHelper pcapHelper2;
  Ptr<PcapFileWrapper> file2 = pcapHelper2.CreateFile ("Third_conf1_2.pcap", std::ios::out, PcapHelper::DLT_PPP);
  devices1.Get (1)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop, file2));

  PcapHelper pcapHelper3;
  Ptr<PcapFileWrapper> file3 = pcapHelper3.CreateFile ("Third_conf1_3.pcap", std::ios::out, PcapHelper::DLT_PPP);
  devices2.Get (1)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop, file3));
  

  Simulator::Stop (Seconds (30));
  Simulator::Run ();
  NS_LOG_UNCOND("Drop Packets "<<drop_count);
  Simulator::Destroy ();
